...
public class OthelloState {
    ...
    /**
     * Returns a better evaluation score of the score of the state.
     * Each piece will be given a score ranging from 1 to 8.  A piece will be given 8 if it 
     * is surrounded by pieces for the same player in all possible directions
     * (top, top-right, right, right-bottom, bottom, bottom-left, left, left-top)
     * @return integer from 1-8
     */
    public int customEval(){
        ...
        for(int i=0; i<boardSize; i++){
            for(int j=0; j<boardSize; j++){
                score = score + pieceScoreEval(i,j);
            }
        }
    ...
    /**
     * Given a piece, return a score for that piece
     * Each piece will be given a score ranging from 1 to 8.  A piece will be given 8 if it 
     * is surrounded by pieces for the same player in all possible directions
     * (top, top-right, right, right-bottom, bottom, bottom-left, left, left-top)
     * @return 
     */
    public int pieceScoreEval(int i, int j){
        int score = 0;

        int playerPiece = board[i][j];
        // check top
        if(((i-1) >= 0 )  && 
