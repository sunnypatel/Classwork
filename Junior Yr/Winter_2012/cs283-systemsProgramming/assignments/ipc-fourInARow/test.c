...
typedef struct{
	...
	int winner = 0;     // holds the PID of winning process.  hopefully of the parent
	int gameover = 0;   // is the gameover? Just in case there are no possible moves left, in which case game would be considered a tie
} game;
...
int checkAround(int *board[10][10], position *pos, int *currentDirection,	int *lineCount){
	...
	for(direction = 0; direction <=7; direction++){
		if(currentDirection != direction || currentDirect == -1){
			position *newPos = Malloc(sizeof(position));
			if((int d=checkPosLine(board, pos, direction, newPos)) >= 0){
